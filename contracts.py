...
class Payroll:
    ...
    def get_amount(self):
        ...
        if self.contract is not None:
            total_amount += self.contract.get_amount()

        if self.commision is not None:
            total_amount += self.commision.get_amount()
        return total_amount
    ...
    def get_text(self):
        return f'{self.contract.get_text()}{(" and "+self.get_commission_text()) if self.commision is not None else ""}. Their total pay is {self.get_amount()}.'
